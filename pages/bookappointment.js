...
const BookAppoinment = () => {
    ...
    useEffect(() => {
        if (router.isReady) {
            if (router.query != undefined) {
                ...
                let cit = router.query.city[0];
                let sta = router.query.state[0];
                let loca = router.query.location[0];
                setBusinessid(Id)
                setCategory(cate)
                setCity(cit)
                setState(sta)
    ...
    const finalFormOnsubmit = async (e) => {
        ...
        console.log(d);
        try {
            const { data } = await axios.post(`${process.env.DOMAIN_NAME}/api/book-appointment`, d)
            console.log(data)
            if (data.success) {
