    ...
    if (replay === "") {
      ...
      try {
        const { data } = await axios.put(
          `${process.env.DOMAIN_NAME}/api/create-replay/${token}/${category}`,
          d
        );
                  ...
                  return (
                    <div className="col-lg-6 col-md-6" key={rev._id}>
                      <div className="user-review">
                        ...
                        <span className="date">
                          <i className="bx bx-calendar"></i> {reviewDate[2]}{" "}
                          {reviewDate[1]} {reviewDate[3]} - {reviewDate[4]}
                        </span>
                        <p>{rev.customerReview}</p>
                        <p style={{ textAlign: "center" }}>{rev.reply}</p>
