...
const PetTraining = () => {
  ...
  const getBusinessProfile = async (id) => {
    try {
      const { data } = await axios.get(`${process.env.DOMAIN_NAME}/api/business/get-profile/PetFood/${id}`);
  ...
  const addProfileFormSubmit = async (e) => {
    ...
    if (email == "" && businessName == "" &&
      ...
      try {
        const { data } = await axios.put(`${process.env.DOMAIN_NAME}/api/business/update-profile/PetFood/${token}`, d);
        console.log(data)
        if (data.success) {
